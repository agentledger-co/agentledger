...
import { checkEmailTypo } from '@/lib/email-typo';
...
export default function SignupPage() {
  ...
  // Track whether the user already saw a typo warning for this email and
  // chose to submit anyway. If they change the email, the flag resets.
  const lastWarnedEmail = useRef('');
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Check for typos / placeholder domains before hitting Supabase
    const typoCheck = checkEmailTypo(email);
    if (typoCheck) {
      if (typoCheck.type === 'placeholder') {
        setError(typoCheck.message);
        setLoading(false);
        return;
      }
      // Typo detected — show suggestion and auto-correct. If they already
      // saw the warning for this exact email and submitted again, let it through.
      if (lastWarnedEmail.current !== email) {
        lastWarnedEmail.current = email;
        setEmailHint(typoCheck.message);
  ...
  return (
    <div className="min-h-screen bg-[#08080a] text-white flex items-center justify-center">
      <div className="max-w-sm w-full mx-4">
        ...
        {/* Google OAuth — first because most ad traffic prefers it */}
        <button
        ...
        <form onSubmit={handleSignup} className="space-y-3">
          ...
          />
          {emailHint && <p className="text-amber-400 text-[13px]">{emailHint}</p>}
          <input
            type="password"
            value={password}
